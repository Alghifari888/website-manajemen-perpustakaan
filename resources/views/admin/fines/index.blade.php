                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" class="py-3 px-6">Tanggal Bayar</th>
                            ...
                            <tbody>
                                @forelse ($fines as $fine)
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        <td class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $fine->user->name }}</td>
                                        <td class="py-4 px-6">{{ $fine->borrowing->book->title }}</td>
                                        <td class="py-4 px-6">Rp {{ number_format($fine->amount, 0, ',', '.') }}</td>
                                        <td class="py-4 px-6">{{ $fine->paid_at ? $fine->paid_at->format('d-m-Y') : '-' }}</td>
                                        <td class="py-4 px-6">
                                            @php
                                                $status = $fine->status;
                                                $statusClasses = [
                                        ...
                                        <td class="py-4 px-6">
                                            @if($status === 'belum_dibayar')
                                                <form action="{{ route('admin.fines.pay', $fine) }}" method="POST" onsubmit="return confirm('Konfirmasi pembayaran denda ini?');">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Tandai Lunas</button>
                                                </form>
                                            @else
                                                -
                                            @endif
